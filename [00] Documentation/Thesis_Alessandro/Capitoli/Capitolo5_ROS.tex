\chapter{ROS Integration and High-Level Software Architecture}

  \section{High-Level Software Architecture: From ROS1 Noetic to ROS2 Jazzy}
  In the previous stages of the OmniCar project, the high-level software
  architecture was based on \textbf{ROS1 Noetic}. In the current work, the
  platform is being migrated to \textbf{ROS2 Jazzy}, with the goal of building a
  more robust, modular, and maintainable.

  This migration represents an important evolution of the project. ROS2 offers
  several advantages compared to ROS1. First, it uses a DDS-based communication
  layer, which provides more reliable and flexible data exchange between nodes.
  Second, ROS2 improves modularity and scalability, making it easier to manage
  distributed robotic systems that include multiple sensors, control nodes, and
  processing pipelines. Third, ROS2 is better suited for future development, since
  it is the actively supported generation of the ROS ecosystem, while ROS1 has
  reached the end of its lifecycle.

  The migration also made it possible to adopt a \textbf{Raspberry Pi 5} as the
  onboard computer of the platform. Compared to the \textbf{Raspberry Pi 3} used
  in the previous implementation, the Raspberry Pi 5 provides higher computational
  power, better memory performance, and a more suitable hardware platform for the
  execution of a modern ROS2-based architecture. This improvement is particularly relevant for tasks such as odometric computation, localization, lidar, and operations where more computational computation is required

  To manage the kinematics of the omnidirectional robot, process sensor data, and
  supervise high-level control operations, the \textbf{ROS (Robot Operating
  System)} framework was adopted as middleware. ROS enables hardware abstraction
  and communication between software modules through a \textit{publish-subscribe}
  model. In this architecture, independent nodes exchange data through topics and
  services, without direct knowledge of each other's internal implementation. This
  approach ensures a decoupled, flexible, and modular system design.

  \subsection{Migration and Local Implementation}
  The migration from ROS1 Noetic to ROS2 Jazzy was motivated by both architectural
  and practical reasons. From the software point of view, ROS2 provides better
  communication reliability, improved maintainability, and a more modern
  development environment. From the hardware point of view, the migration allows
  the software stack to fully exploit the improved capabilities of the Raspberry
  Pi 5.

  The ROS2 environment was implemented inside a dedicated local \textit{workspace}
  on the Raspberry Pi. This choice was made for the following reasons:
  ...
  libraries and packages.
      \item \textbf{Modularity}: The workspace structure helps organize drivers,
  interfaces, launch files, and configuration files in a clear and maintainable
  way.
      \item \textbf{Portability}: The complete software stack can be deployed more
  easily on different Raspberry Pi boards while preserving the same internal
  package organization.
      \item \textbf{Scalability}: ROS2 makes future extensions easier, such as
  ...
  ESP32 firmware;
      \item the progressive migration of the main functional modules, including
  driver, joystick, odometry, localization, and lidar.
  \end{itemize}

  For these reasons, the migration had to be performed progressively. The first
  priority was the consolidation of the ROS2 driver compatible with the embedded
  firmware. After that, the other software modules could be migrated and
  ...
  Within each package, the \texttt{CMakeLists.txt} and \texttt{package.xml} files
  remain fundamental. The \texttt{CMakeLists.txt} file defines the compilation
  rules, executable targets, and library linking, while the \texttt{package.xml}
  file describes the package metadata and declares the required dependencies.
...
\end{folderbox}
 ...
 \subsection{Experimental Validation: ROS2 Integration and Control Chain}
  The final phase of the software integration focused on validating the
  ...
  the ESP32 controller, the consistency of encoder feedback, and the proper
  integration of teleoperation, odometry, and localization modules inside the
  migrated ROS2 architecture.

  \subsubsection{Test 1: Encoder Monitoring and Open-Loop Velocity Control}
  The first experimental test was designed to validate the behaviour of the main
  ROS2 driver and the correctness of the encoder data received from the embedded
  controller.
  ...
  It also exposes the services \texttt{set\_solenoid\_1\_state} and
  \texttt{set\_solenoid\_2\_state}.
...
\end{figure}
  ...
  \subsubsection{Test 2: Teleoperation via Joystick Control}
  The second validation phase concerned the integration of joystick teleoperation
  ...
  This result is important from an architectural point of view, because it shows
  that the migrated ROS2 control chain is modular and well separated into
  functional layers. The joystick node manages the user input, the kinematic
  conversion node computes the wheel references, and the main driver handles
...
The comparison between the real experiment and the RViz visualization shows that the odometry provides a consistent estimate of the robot motion. The trajectory and frame alignment are coherent, while small differences are due to odometric drift, wheel slip, and modeling errors.
 ...
 \begin{figure}[H]
    ...
    \begin{minipage}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{IMG/ROS/rviz2_odom_edit.png}
        \caption*{RViz odometry and TF visualization}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{IMG/ROS/odom_real_edit.jpg}
...
\end{figure}
  ...
  \subsubsection{Test 4: Localization and Lidar Integration}
  ...
  the lidar subsystem. (TODO, possibile argomento extra sulla sensor fusion e del ExtendedKalmanFilter filter, non sviluppato da me)
...
\newpage
\subsubsection{Test 5: Waypoint Tracking Validation with the
  \texttt{go\_to\_point} Controller}

  This is the first closed-loop
  high-level motion control test in the ROS2 architecture. In this phase, the
  package \texttt{sdpo\_motion\_control} was used to command the robot toward a
  sequence of planar target poses, with the objective of evaluating the
  behaviour
  of the \texttt{go\_to\_point} controller, the consistency of the odometric
  feedback, and the overall quality of the integrated control chain.
  ...
  the
  previous implementation, the rotation between \texttt{map} and \texttt{odom}
  was
  computed correctly, but the translation term was combined with the robot EKF
  yaw
  instead of the relative \texttt{map}--\texttt{odom} rotation. This produced a
  frame inconsistency. After this correction, the waypoint experiments
  became coherent and repeatable.

  \paragraph{Test configuration}
  For controller tuning and validation, five reference poses were assigned in
  sequence. The same five target positions are the ones shown in the plots of
...
\end{itemize}
  ...
  These references were chosen to test both pure translations and combined
  translation-rotation motions over a significant portion of the available
  working
  area, ending with a return to the origin.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{IMG/ROS/TEST5_position.png}
    \caption{plot control position results}
    \label{fig:TEST5_position}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{IMG/ROS/TEST5_position_error.png}
    \caption{plot position errors}
    \label{fig:TEST5_position_error}
\end{figure}

  The comparison between the commanded target pose and the estimated current
  pose,
  together with the plots of the tracking error, highlighted that the control
  loop
  remained stable during all five tests, but also confirmed the presence of a
  residual positioning error. In the experimental trials, the maximum observed
  deviation with respect to the real robot position was approximately a few
  centimetres, with worst cases on the order of about \(2 \ \mathrm{cm}\) on each
  ...
  and
  for future trajectory-tracking developments.

  At the same time, the tests showed that improved precision will require not
  only
  controller tuning, but also a more accurate calibration of the localization
  and
  odometry chain, especially for diagonal motions and global return-to-origin
  tasks. For this reason, Test~5 was an important milestone: it validated the
  complete waypoint control loop in ROS2 and clearly identified the remaining
  gap
  between acceptable closed-loop operation and higher-accuracy autonomous
  navigation.
