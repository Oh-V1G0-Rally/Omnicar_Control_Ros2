...
\begin{equation}
    ...
    \end{bmatrix}^{T},
\end{equation}
where \(x\) and \(y\) are the robot position and \(\psi\) is the yaw angle. The first two controllers are geometric and non-optimizing, they do not directly solve an optimization problem. Instead, they use geometric quantities such as path projection, path tangent, cross-track error, and along-track error. The third one (MPCC) is optimization-based.
...
in \texttt{Test/test\_results/}.

\vspace{0.2cm}

The main performance indicators are computed from the sampled tracking errors.
For a generic error signal \(e_i\), with \(N\) samples, we can calculate:
...
\begin{equation}
    e_{\psi}
    =
    \operatorname{wrap}_{[-\pi,\pi]}\!\left(\psi_{r}-\psi\right),
    \qquad
    e_{\psi}\in[-\pi,\pi].
\end{equation}
$\operatorname{wrap}_{[-\pi,\pi]}$ maps any angle to its principal
value in $[-\pi,\pi]$, so that the heading controller uses the shortest angular error.
\newline
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{IMG/controller/MREFP/MR_R01_xy_map.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=0.7\linewidth]{IMG/controller/MREFP/MR_R01_target_tracking_and_errors.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{IMG/controller/MREFP/PR01_xy_map.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{IMG/controller/MREFP/PR02_xy_map.png}
...
This section describes the predictive controller used in the experimental tests.  The objective of the controller is not only to
...
\begin{aligned}
    ...
    \psi_{k+1} &= \operatorname{wrap}_{[-\pi,\pi]}
    \!\left(\psi_k + \Delta t\,\omega_k\right), \\
...
\begin{equation}
    ...
    \!\left(\psi_r(\gamma_k)-\psi_k\right),
    \qquad
    e_{\psi,k}\in[-\pi,\pi].
\end{equation}
...
absolute velocity to be unnecessarily small.  

\begin{comment}
A terminal cost is also added to
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=0.6\linewidth]{IMG/controller/MPC/MPC_01_track_error.png}
...
LIN-05 achieves the best contouring performance. Increasing the velocity in
LIN-06 reduces the duration from \(50.14~\mathrm{s}\) to
...
The second comparison considers two MPCC runs. MPC-05 provides better geometric
tracking, with \(\mathrm{RMSE}_{e_c}=0.0170~\mathrm{m}\) and
\(\max |e_c|=0.0357~\mathrm{m}\). MPC-06 is faster, completing the common
...
The final cross-comparison evaluates the aggressive Linear Segment run LIN-05
against MPC-06 over the same \(4.534~\mathrm{m}\) common segment. In this case,
MPC-06 achieves better lateral path-following accuracy:
\[
    \mathrm{RMSE}_{e_c}
    =
    0.0330~\mathrm{m}
    ...
    0.0469~\mathrm{m}
\]
for LIN-05. The maximum contouring error is also lower for MPC-06:
\[
    \max |e_c|
    =
    0.0705~\mathrm{m}
    ...
    0.1166~\mathrm{m}.
\]
However, the lag error remains much larger for MPC-06
