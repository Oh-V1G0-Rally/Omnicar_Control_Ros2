...
for _ in range(int(200 * 1 / rob.dt_control)):
    rob.step()
    ...
    v_actual, w_actual = rob.get_control()
    v_log.append(v_actual)
    w_log.append(w_actual)

    # Reference values (computed in rob.ref)
    vref_log.append(rob.ref[0])
    wref_log.append(rob.ref[1])
...
ani = animation.FuncAnimation(
    ...
    frames=len(x_sampled), interval=rob.dt_control * 50 * sample_rate,
    blit=True
)
