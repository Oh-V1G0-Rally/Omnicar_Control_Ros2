...
p_gamma = path.get_p_gamma_upper_bound()
u1_bounds=(-4,4)
u2_bounds=(-0.5,0.5)
...
ax.set_ylim(min(x_log) - 1, max(x_log) + 1)
ax.set_xlabel("Y (m)")
ax.set_ylabel("X (m)")
ax.set_title("Vehicle Path Under the PCBF Min-Norm Controller")
ax.grid(True)

# Main Plot
ax.plot(y_log, x_log, label="Vehicle Path", linewidth=2)
ax.plot(left_y, left_x, 'r--', label=r'$\partial \mathcal{C}$')
ax.plot(right_y, right_x, 'r--')
ax.plot(left_y2, left_x2, 'k--', label=r'$\partial \mathcal{C}_\delta$')
ax.plot(right_y2, right_x2, 'k--')
...
# zoom=4 scales the inset. Adjust this if it looks too big/small.
# loc='center right' places the box. You can try 'lower right', 'upper right', etc.
axins = ax.inset_axes([0.55, 0.1, 0.15, 0.3])

# 2. Re-plot all the same data on the inset
axins.plot(y_log, x_log, linewidth=2)
axins.plot(left_y, left_x, 'r--')
axins.plot(right_y, right_x, 'r--')
axins.plot(left_y2, left_x2, 'k--')
axins.plot(right_y2, right_x2, 'k--')

# 3. Set the limits for the zoomed region (based on your image 2)
# Note: Your axes are labeled X but use y_log data, and vice versa.
x_zoom_min, x_zoom_max = 10, 13#-18, -17.3  # Y-axis in reality
y_zoom_min, y_zoom_max = 63.2, 63.6#-1.7, -0.7    # X-axis in reality

axins.set_ylim(x_zoom_min, x_zoom_max)
axins.set_xlim(y_zoom_min, y_zoom_max)
axins.grid(True)

# Optional: Hide tick labels on inset if it gets too cluttered
axins.set_xticklabels([])
axins.set_yticklabels([])

# 4. Draw the connecting lines
# loc1 and loc2 dictate which corners connect (1=UR, 2=UL, 3=LL, 4=LR)
...
#
# # Plot δ(t) on the left y-axis
# ax1.plot(t_log, delta_log, label=r'$u_\gamma(t)$', color='C0', linewidth=2)
# ax1.axhline(y=delta_lb*460, color='C0', linestyle='--', linewidth=1.2, alpha=0.8, label=r'$u_{\gamma min}$')
# ax1.set_xlabel(r'Time (s)', fontsize=12)
# ax1.set_ylabel(r'$u_\gamma$', color='C0', fontsize=12)
# ax1.tick_params(axis='y', labelcolor='C0')
# ax1.grid(True, which='both', linestyle='--', linewidth=0.7, alpha=0.7)
#
# # Create right y-axis for u(t)
# ax2 = ax1.twinx()
# ax2.plot(t_log, v_log, label=r'$v(t)$', color='C3', linewidth=1)
# ax2.set_ylabel(r'$b$', color='C3', fontsize=12)
# ax2.plot(t_log, w_log, label=r'$\omega(t)$', color='C1', linewidth=1)
# ax2.set_ylabel(r'$v(t) \text{ and } \omega(t)$', color='C3', fontsize=12)
# ax2.tick_params(axis='y', labelcolor='C3')
#
# # Title and legends
# fig.suptitle(r'Path Speed adjustment $u_\gamma(t)$ and Control Input $u(t)$ Over Time', fontsize=14, fontweight='bold')
#
# # Combine legends from both axes
# lines_1, labels_1 = ax1.get_legend_handles_labels()
# lines_2, labels_2 = ax2.get_legend_handles_labels()
# ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='best', frameon=True, fontsize=10)
#
# plt.tight_layout()
# plt.show()
