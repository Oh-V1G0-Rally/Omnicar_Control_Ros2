...
void OdomWhMotorDrive::setEncTicksDelta(const int32_t & delta_ticks)
{
  ...
  ang_delta = inverted ? -enc_ticks_delta * 2.0 * M_PI / ticks_per_rev :
    enc_ticks_delta * 2.0 * M_PI / ticks_per_rev;

  dist_delta = ang2lin(ang_delta);
}

void OdomWhMotorDrive::setEncTicksDelta(const int32_t & delta_ticks, const double & ticks_rev)
{
...
void OdomWhMotorDrive::setDistDelta(const double & delta_dist)
{
  ...
  enc_ticks_delta = inverted ?
    -std::round(ang_delta * ticks_per_rev / (2.0 * M_PI)) :
    std::round(ang_delta * ticks_per_rev / (2.0 * M_PI));
}

void OdomWhMotorDrive::setDistDelta(const double & delta_dist, const double & ticks_rev)
{
  ticks_per_rev = ticks_rev;
