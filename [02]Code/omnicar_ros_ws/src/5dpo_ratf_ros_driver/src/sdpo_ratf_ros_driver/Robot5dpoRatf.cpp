...
void Robot5dpoRatf::rcvSerialData(const char *data, unsigned int len) {
  ...
  for(auto& c : vec) {
    // printf("DEBUG SERIALE: Ricevuto carattere [%c] (hex: %02x)\n", c, (unsigned char)c); // DEBUG
    //printf("%c", c); // DEBUG
...
//   mtx_.lock();
//   float w_ref_float[4];
//   w_ref_float[0] = static_cast<float>(mot[0].w_r);
//   w_ref_float[1] = static_cast<float>(mot[1].w_r);
//   w_ref_float[2] = static_cast<float>(mot[2].w_r);
//   w_ref_float[3] = static_cast<float>(mot[3].w_r);
//   mtx_.unlock();

//   char buffer[15]; 
//   char channels[4] = {'G', 'H', 'I', 'J'};

//   for (int i = 0; i < 4; i++) {
//     uint32_t raw_bits;
//     // Copiamo i bit del float nell'intero a 32 bit senza cambiarli
//     std::memcpy(&raw_bits, &w_ref_float[i], sizeof(float));
    
//    printf("INVIO SERIALE: %s", buffer);
// 
//     char cmd[16];
//    int len = snprintf(cmd, sizeof(cmd), "%c%08X\n", channels[i], raw_bits);

//     if (len > 0) {
//         // printf("INVIO SERIALE: %s", cmd); // Debug pulito
//         serial_async_->writeString(std::string(cmd));
//     }
//   }
// }
